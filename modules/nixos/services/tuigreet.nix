{
  ...
  falsy-data = dat: let t = builtins.typeOf dat; in
    if t == "list"
      then join " " dat
    else if t == "set"
      ...
      then if dat then " " else ""
    else builtins.toString dat;

  mk-tuigreet-options = opts: lib.foldl'
    (x: y: if y.value != "" then "--${y.name} ${builtins.toString y.value} ${x}" else "${x}")
    ""
    (lib.mapAttrsToList 
      (a: b: {name = a; value = falsy-data b;})
      opts
    );
...
in {
  options.eula.modules.services.tuigreet = {
    ...
    settings = mkOpt (types.submodule { options = {
